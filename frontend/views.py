...
# Paths and Keys
COOKBOOK_PATH = os.path.join("data", "cookbook_data.json")
...
# === Grocery List for a Single Recipe (API) ===
...
# === Weekly Grocery List (Combined) API ===
...
# === Instruction Delivery Page (View Version) ===
def instruction_delivery_view(request):
    if not request.session.get("cookbook_uploaded") or not os.path.exists(COOKBOOK_PATH):
        messages.error(request, "Please upload a cookbook first.")
        return render(request, "upload.html")

    return render(request, "instruction_delivery.html", {
        "api_key": EXPECTED_API_KEY
    })

# === Weekly Cooking Instructions (AJAX API Version) ===
def weekly_instructions_view(request):
    ...
    try:
        ...
        return JsonResponse({"weekly_instructions": weekly_instructions})
    except Exception as e:
        print(f"[❌] Error generating weekly instructions: {e}")
        return JsonResponse({"error": "Failed to generate instructions"}, status=500)
